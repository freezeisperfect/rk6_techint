  ...
  static async addTask({
    ...
    const result = await fetch(
      `http://localhost:4321/tasklists/${tasklistId}/tasks`,
      {
        ...
        body: JSON.stringify({ taskName, taskReason })
      }
    );
...
class App {
  ...
  moveTask = async ({ taskID, direction }) => {
    ...
    const taskName = this.tasklists[tlIndex].tasks[taskIndex]?.taskName;
    const taskReason = this.tasklists[tlIndex].tasks[taskIndex]?.taskReason;
    ...
    try {
      ...
      this.tasklists[targetTlIndex].addTask(taskName, taskReason);
      console.log(this.tasklists[tlIndex].tasks)
    } catch (error) {
...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите ФИО нового клинта, которому необходимо будет позвонить:');

    if (!newTaskName) return;

    const newTaskReason = prompt('Введите причину звонка');

    if (!newTaskReason) return;
    ...
    try {
      await AppModel.addTask({
        ...
        taskReason: newTaskReason,
      });
      this.addTask(newTaskName, newTaskReason);
    } catch (error) {
      console.error('ERROR', error);
    }
  };

  addTask = (taskName, taskReason) => {
    ...
    this.tasks.push({taskName, taskReason});
  };

  onEditTask = async (taskID) => {
    const taskIndex = Number(taskID.split('-T')[1]);
    const oldTaskName = this.tasks[taskIndex].taskName;
    ...
    try {
      ...
      this.tasks[taskIndex].taskName = newTaskName;
      document.querySelector(`#${taskID} .task-text-container .tm-tasklist-task-text`)
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Клиент '${taskName}' будет удален из списка. Продолжить?`)) return;
  ...
  rerenderTasks = () => {
    ...
    this.tasks.forEach((taskItem, taskIndex) => {
      tasklist.appendChild(
        this.renderTask({
          taskID: `${this.tlID}-T${taskIndex}`,
          taskName: taskItem?.taskName,
          taskReason: taskItem?.taskReason,
        })
      );
    });
  };

  renderTask = ({ taskID, taskName, taskReason }) => {
    const task = document.createElement('li');
    task.classList.add('tm-tasklist-task');
    task.id = taskID;

    const container = document.createElement('div');
    container.classList.add('task-text-container');
    ...
    reason.classList.add('task-reason');
    reason.innerHTML = taskReason;
  ...
  render() {
    ...
    button.innerHTML = 'Добавить клиента на обзвон';
