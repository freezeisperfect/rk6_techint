...
app.post('/tasklists/:tasklistId/tasks', async (request, response) => {
  const { taskName, taskReason } = request.body;
  ...
  tasklists[tasklistId].tasks.push({taskName, taskReason});
  await writeData(tasklists);
  response
    .setHeader('Content-Type', 'application/json')
    .status(200)
...
app.put('/tasklists/:tasklistId/tasks/:taskId', async (request, response) => {
  ...
  tasklists[tasklistId].tasks[taskId].taskName = newTaskName;
  await writeData(tasklists);
  response
    .setHeader('Content-Type', 'application/json')
    .status(200)
...
app.patch('/tasklists/:tasklistId', async (request, response) => {
  ...
  const movedTask = tasklists[fromTasklistId].tasks[taskId];

  tasklists[fromTasklistId].tasks.splice(taskId, 1);
  tasklists[toTasklistId].tasks.push(movedTask);
  await writeData(tasklists);
  response
    .setHeader('Content-Type', 'application/json')
    .status(200)
    .json({
      info: `Task '${movedTask}' was successfully moved from tasklist '${tasklists[fromTasklistId].tasklistName}' to tasklist '${
